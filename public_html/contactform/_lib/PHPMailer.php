...
class PHPMailer
{
    ...
    /**
     * Whether to throw ExceptionMails for errors.
     *
     * @var bool
     */
    protected $ExceptionMails = false;
     ...
     * Constructor.
     *
     * @param bool $ExceptionMails Should we throw external ExceptionMails?
     */
    public function __construct($ExceptionMails = null)
    {
        if (null !== $ExceptionMails) {
            $this->ExceptionMails = (bool) $ExceptionMails;
        }
     ...
     * Addresses that have been added already return false, but do not throw ExceptionMails.
     *
     * @param string $kind    One of 'to', 'cc', 'bcc', or 'ReplyTo'
     * @param string $address The email address to send, resp. to reply to
     * @param string $name
     *
     * @throws ExceptionMail
     *
     * @return bool true on success, false if address already used or invalid in some way
     */
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->ExceptionMails) {
                throw new ExceptionMail($error_message);
            }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new ExceptionMail($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new ExceptionMail($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new ExceptionMail($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new ExceptionMail($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new ExceptionMail($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new ExceptionMail($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new ExceptionMail($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw new ExceptionMail($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new ExceptionMail($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new ExceptionMail($this->lang('authenticate'));
                        }
                    }

                    return true;
                } catch (ExceptionMail $exc) {
                    $lastExceptionMail = $exc;
                    $this->edebug($exc->getMessage());
        ...
        $this->smtp->close();
        // As we've caught all ExceptionMails, just report whatever the last one was
        if ($this->ExceptionMails and null !== $lastExceptionMail) {
            throw $lastExceptionMail;
        }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new ExceptionMail($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new ExceptionMail($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new ExceptionMail($this->lang('signing') . openssl_error_string());
                }
            } catch (ExceptionMail $exc) {
                $body = '';
                if ($this->ExceptionMails) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new ExceptionMail($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!file_exists($path)) {
                throw new ExceptionMail($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
    ...
    public function DKIM_Sign($signHeader)
    {
        if (!defined('PKCS7_TEXT')) {
            if ($this->ExceptionMails) {
                throw new ExceptionMailMail($this->lang('extension_missing') . 'openssl');
            }
